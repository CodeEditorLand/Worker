const s = typeof window._WORKER == "string" ? window._WORKER : "/Worker.js",
	_ = "/Application",
	e = "WorkerReload";
const l = () => {};
if ("serviceWorker" in navigator) {
	(navigator.serviceWorker.addEventListener("controllerchange", () => {
		sessionStorage.getItem(e) === "true" &&
			(sessionStorage.removeItem(e), window.location.reload());
	}),
		navigator.serviceWorker.addEventListener("message", (r) => {
			r.data?.Version === "New" && window.location.reload();
		}));
	const i = async () => {
		...
		try {
			...
			if (window.trustedTypes)
				try {
					...
					o = t.createScriptURL(s);
				} catch (t) {
					throw t;
				}
			else o = s;
			const n = await navigator.serviceWorker.register(o, {
				scope: _,
				type: "module",
			});
			(n.installing || n.waiting || n.active,
			...
			else sessionStorage.getItem(e) && sessionStorage.removeItem(e);
		} catch (o) {
			(o instanceof TypeError &&
				(o.message.includes("TrustedScriptURL") ||
					o.message.includes("Trusted Type")),
